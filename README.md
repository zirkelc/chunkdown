...
Preserving a complete semantic unit like a section, paragraph, sentence, etc., is often more important than adhering to a strict chunk size. The splitter allows a controlled overflow (via `maxOverflowRatio`) of the chunk size if it avoids splitting a complete unit.
...
The splitter uses a multi-phase approach:

1. **AST Parsing**

    The markdown text is parsed into an Abstract Syntax Tree using [`mdast-util-from-markdown`](https://github.com/syntax-tree/mdast-util-from-markdown). This provides semantic understanding of every element in the document.

2. **Hierarchical Transformation**

    The flat AST is transformed into a hierarchical structure where:
    - Headings become section nodes
    - Content following a heading belongs to that section
    - Nested headings create nested sections
    - Orphaned content (before first heading) creates shadow sections

3. **Top-Down Chunking**

    Sections are processed top-down with these strategies:
    - **Overflow Ratio**: Allow controlled overflow of chunk size (via `maxOverflowRatio`) to keep related content together
    - **Section Grouping**: Combine consecutive small sections to maximize chunk utilization
    - **Content Aggregation**: Group paragraphs, lists, tables and other content efficiently
