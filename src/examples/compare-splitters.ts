...
const colors = [chalk.cyan, chalk.yellow, chalk.green, chalk.magenta, chalk.blue, chalk.red];

/** Consistent colors for each splitter (neutral) */
const chunkdown = chalk.bold(`chunkdown`);
const langchain = chalk.bold(`langchain`);
...
/** Parse --index argument to run a specific example */
const args = process.argv.slice(2);
const indexArg = args.find((arg) => arg.startsWith(`--index=`));
const filterIndex = indexArg ? parseInt(indexArg.split(`=`)[1], 10) : undefined;
...
(async () => {
  ...
  console.log(
    [
      chalk.bold.dim(`Comparison`),
      chalk.bold(`chunkdown`),
      chalk.bold.dim(`vs`),
      chalk.bold(`@langchain/textsplitters`),
    ].join(` `),
  );
  console.log(`${'─'.repeat(80)}`);
  console.log();
  ...
  for (let exampleIdx = 0; exampleIdx < examples.length; exampleIdx++) {
    if (filterIndex !== undefined && exampleIdx !== filterIndex) continue;
    ...
    // console.log(`${"─".repeat(80)}`);
    // console.log(`${chalk.dim.bold(`Test #${exampleIdx} ${example.name}`)}`);
    // console.log(`${"─".repeat(80)}`);
    // console.log();
    ...
    console.log(`${chalk.dim(`Chunk size: `)}${example.chunkSize}`);
    console.log();
    // console.log(`  Chunk size: ${example.chunkSize}`);
    ...
    console.log(``);
    console.log(chalk.dim(`Mapping (plain-to-markdown):`));
